...
function Shelveavail(): React.JSX.Element {
  ...
  const fetchShelves = async (searchQuery: string = ""): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.get<ShelfItem[]>(`/api/shelves?search=${encodeURIComponent(searchQuery)}`, {
  ...
  const handleDelete = async (shelfNumber: string): Promise<void> => {
    ...
    const token = localStorage.getItem("jwtToken");
    try {
      const response = await axios.delete(`/api/shelves/${encodeURIComponent(shelfNumber)}`, {
